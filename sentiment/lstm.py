...
Copyright: https://gist.github.com/karpathy/587454dc0146a6ae21fc
"""
import numpy as np
import code
  ...
  for j in range(len(tocheck)):
    ...
    for i in range(mat.size):
      ...
      print('%s checking param %s index %s (val = %+8f), analytic = %+8f, numerical = %+8f, relative error = %+8f' \
            % (status, name, repr(np.unravel_index(i, mat.shape)), old_val, grad_analytic, grad_numerical, rel_error))
